layui.use(['table','layer'],function(){
    ...
    var tableIns = table.render({
        ...
        ,cols: [[
            ...
            ,{field: 'uPhone', title: '用户号码', align:'center'}
            ,{field: 'uEmil', title: '用户邮箱', align:'center'}
            ,{field: 'uProfession', title: '部门专业', align: 'center'}
            ,{title:'操作',templet:'#adminListBar', fixed: 'right', align:'center', minWidth:150}
        ]]
    });
     ...
     * 监听头部工具栏
     */
    /*table.on('toolbar(admins)', function (data) {
     ...
     * 监听行工具栏
     */
    /*table.on('tool(admins)', function (data) {
...
*/
   ...
   /* function deleteUser(id) {
        // 弹出确认框，询问用户是否确认删除
        layer.confirm('确定要删除该记录吗？',{icon:3, title:"管理员管理"}, function (index) {
            // 关闭确认框
            layer.close(index);
    ...
    /*function openAddOrUpdateUserDialog(id) {
        ...
        var url = ctx + "/admin/toAddOrUpdateUserPage";

        // 判断id是否为空；如果为空，则为添加操作，否则是修改操作
        if (id != null && id != '') {
            title = "<h3>用户管理 - 更新用户</h3>";
            url+= "?uId="+id; // 传递主键，查询数据
        }
